...
module.exports = {
    ...
    // },
    getNextListUrlChunk: function(list, callback) {
        this.postJson(list, '/api/next_chunk', callback);
    },

    getNextPostUrlChunk: function(data, callback) {
        this.postJson(data, '/api/next_post_url_chunk', callback);
    },
        ...
        if (/mp\/getmasssendmsg/i.test(req.url)) {//当链接地址为公众号历史消息页面时(第一种页面形式)
            if (serverResData.toString() !== "") {
                try {//防止报错退出程序
                    ...
                    this.getNextListUrlChunk(list, function(nextChunk) {
            ...
            try {
                var data = {};
                data.current_url = req.url;
                this.getNextPostUrlChunk(data, function(nextChunk) {
                    serverResData += nextChunk;
                    callback(serverResData);
                });
            }
